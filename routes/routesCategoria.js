const express = require('express');
const {MongoClient, ObjectId} = require('mongodb');
const categoriaService = require('../services/categoriaService.js');
const router = express.Router();
const service = new categoriaService;

    router.get('/', async (req,res)=>{
        const categorias = await service.find();
            if (categorias){
                res.status(201).send(categorias)
            }else{
                res.status(404).send('No se encontraro');
            }
    })

    router.get('/:id', async (req,res)=>{
        const id = req.params.id;
        const categoria = await service.findOne(id);
        if(categoria){
            res.status(200).send(categoria);
        }else{
            res.status(404).send(`No se encontró el categoria con el id: ${id}`);
        }
    })

    router.post('/addcategoria', async (req,res)=>{
        const result = await service.add(req.body);
        if(result){
            res.status(200).json({
                message:`Se agregó un categoria`
            })
        }else{
            res.status(404).send('Error al intentar insertar categorias')
        }
    })

    router.post('/addcategorias', async (req,res)=>{
        const result = await service.addMany(req.body);
        if(result){
            res.status(200).json({
                message:'Se han guardado los categorias correctamente'
            });
        }else{
            res.status(404).send('Error al intentar insertar categorias');
        }
    })

    router.patch('/:id', async (req,res)=>{
        const id = req.params.id;
        const {nombre, estado} = req.body;
        const result = await service.update(id,nombre,estado);
        if(result.modifiedCount > 0 ){
            res.status(200).json({
                message:`El categoria con el id: ${id} ha sido actualizado`
            });
        }else{
            res.status(404).send('Error al intentar actualizar el categoria');
        }
    })

    router.patch('/updatecategorias/:campoCond/:valorCond/', async (req,res)=>{
        const {campoCond,valorCond} = req.params;
        const {campoUpdate,valorUpdate} = req.body;
        const condicion = `{"${campoCond}":"${valorCond}"}`;
        const actualizacion = `{"${campoUpdate}":"${valorUpdate}"}`;
        const result = await service.updateMany(condicion,actualizacion);
        if(result){
            res.status(200).json({
                message:`Se actualizaron los mateiales con la condición ${condicion} en la base de datos`
            });
        }else{
            res.status(404).send('error al intentar actualizar categorias');
        }
    })

    router.delete('/:id', async (req,res)=>{
        const id = req.params.id;
        const result = await service.delete(id);
        if (result.acknowledged == true && result.deletedCount > 0 ){
            res.status(200).json({
                message:`categoria eliminado exitosamente`,
            });
        }else{
            res.status(404).send('No se pudo eliminar el categoria')
        }
    })

    router.delete('/deletecategorias/:campo/:valor', async (req,res)=>{
        const {campo, valor} = req.params;
        const condicion = campo+":"+valor;
        const result = await service.delteMany(condicion);
        console.dir(result);
        console.dir(condicion);
        if(result.acknowledged == true && result.deletedCount > 0 ){
            res.status(200).json({
                message:`Se han borrado todos los registros que cumplen con la condición ${condicion}`
            });
        }else{
            res.status(404).send('Error al intentar eliminar los categorias');
        }
    })

module.exports = router;